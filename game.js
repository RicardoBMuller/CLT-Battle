/* =========================================================
   A BATALHA DO CLT — Vertical Layout + VirtualPad
   - Canvas vai para #game-shell (top)
   - Controles touch embaixo via window.VirtualPad
   ========================================================= */
...
/* =========================================================
   ...
   ========================================================= */
const VPad = {
  get(){ return window.VirtualPad || {up:false,down:false,left:false,right:false,a:false,b:false,start:false,select:false}; },
  // "JustDown" (edge)
  _prev: {up:false,down:false,left:false,right:false,a:false,b:false,start:false,select:false},
  justDown(key){
    const now = !!this.get()[key];
    const was = !!this._prev[key];
    return now && !was;
  },
  syncPrev(){
    const cur = this.get();
    for (const k of Object.keys(this._prev)) this._prev[k] = !!cur[k];
  }
};
...
/* =========================================================
   XP curve / profissões
   ========================================================= */
function xpForNextLevel(level){ return 60 + Math.floor(Math.pow(level,1.6)*45); }
...
/* =========================================================
   AUDIO (igual ao seu, volume ok)
   ========================================================= */
const Audio = {
  ctx:null, master:null, musicGain:null, sfxGain:null,
  started:false, musicTimer:null, musicType:"none",
  ...
  sfx(name){
    ...
    switch(name){
      case "ui_move":  this.tone(660,0.05,"square",0.20, 520); break;
      case "ui_ok":    this.tone(880,0.07,"square",0.30); this.tone(1320,0.06,"square",0.22); break;
      case "ui_back":  this.tone(420,0.07,"square",0.26, 320); break;
      case "step":     this.tone(180,0.03,"triangle",0.16, 140); break;
      case "encounter":this.noise(0.07,0.20); this.tone(220,0.12,"square",0.26, 440); break;
      case "hit":      this.noise(0.05,0.30); this.tone(220,0.07,"square",0.26, 120); break;
      case "heal":     this.tone(523,0.08,"triangle",0.28); this.tone(659,0.08,"triangle",0.24); break;
      case "win":      this.tone(659,0.10,"square",0.26); this.tone(784,0.10,"square",0.26); this.tone(988,0.12,"square",0.26); break;
      case "lose":     this.tone(196,0.14,"square",0.28,110); this.noise(0.08,0.22); break;
      case "attack":   this.tone(392,0.05,"square",0.28); this.noise(0.03,0.16); break;
      case "crit":     this.tone(1318,0.06,"square",0.30); this.tone(1760,0.06,"square",0.28); break;
      case "coin":     this.tone(988,0.05,"square",0.26); this.tone(1244,0.05,"square",0.22); break;
      case "spark":    this.tone(1046,0.06,"triangle",0.28); this.tone(1568,0.08,"triangle",0.22); break;
    }
  },
  ...
  playMusic(type){
    ...
    const serene = { bpm:92, lead:[72,74,76,74,72,69,67,69,72,74,76,79,76,74,72,69], bass:[48,48,55,55,50,50,57,57] };
    const battle = { bpm:146, lead:[76,79,83,79,76,74,71,74,76,79,83,86,83,79,76,74], bass:[43,43,46,46,41,41,46,46] };
    ...
    this.musicTimer = setInterval(()=>{
      ...
      if (step % 4 === 0) playNote(freq(nBass), stepDur*1.4, "triangle", 0.085);
      playNote(freq(nLead), stepDur*0.95, "square", (type==="battle") ? 0.11 : 0.08);
      if (type==="battle" && step % 2 === 0) this.noise(0.02, 0.07);
      step++;
    }, stepDur * 1000);
  }
};
...
function makeEnemy(scene, keyBase, style){
  for (let f=0; f<2; f++){
    makeTex(scene, `${keyBase}_${f}`, 56, 56, (g)=>{
      ...
      g.fillStyle(style.aura,0.10); g.fillCircle(28,30,24);
      g.fillStyle(0x8ecae6,0.30); g.fillCircle(28,30,18);
      g.fillStyle(0x5fa8d3,0.58); g.fillRoundedRect(14,16,28,30,12);
      g.fillStyle(0xffffff,0.08); g.fillCircle(22,22,6);

      if (style.hood){
        g.fillStyle(PALETTE.ink,0.55); g.fillRoundedRect(13,14,30,18,8);
        g.fillStyle(PALETTE.ink,0.75); g.fillRect(18,20,20,8);
        g.fillStyle(0xffffff,0.08);
        g.fillRect(24,28,1,6); g.fillRect(31,28,1,6);
      }
      if (style.bag){
        g.fillStyle(0xff4d4d,0.55); g.fillRoundedRect(34,22,14,16,3);
        g.fillStyle(PALETTE.ink,0.25); g.fillRect(36,24,10,12);
        g.fillStyle(0xffffff,0.09); g.fillRect(37,28,8,2);
        g.fillStyle(PALETTE.ink,0.35); g.fillRect(30,22,4,2);
      }

      g.fillStyle(PALETTE.ink,0.9);
      if (style.eyes === "mean"){ g.fillRect(20,28,8,1); g.fillRect(30,28,8,1); }
      g.fillRect(22,30,4,4); g.fillRect(32,30,4,4);

      if (style.knife){
        g.fillStyle(0xffffff,0.14); g.fillRect(40,38,6,1);
        g.fillStyle(PALETTE.ink,0.25); g.fillRect(39,39,3,2);
      }
      g.fillStyle(0xff6b6b,0.65); g.fillRect(25,40,6,(f===0?2:3));
...
function genArt(scene){
  makeTex(scene,"tile_ground",TILE,TILE,(g)=>{
    ...
    for(let i=0;i<7;i++){ g.fillStyle(shade(PALETTE.grass3,0.10),0.35); g.fillRect(rint(1,14),rint(1,14),1,1); }
  ...
  makeTex(scene,"tile_grass",TILE,TILE,(g)=>{
    ...
    for(let i=0;i<11;i++){ g.fillStyle(shade(PALETTE.grass1,0.15),0.40); g.fillRect(rint(1,14),rint(2,14),1,rint(1,2)); }
  ...
  makeTex(scene,"tile_path",TILE,TILE,(g)=>{
    ...
    for(let i=0;i<10;i++){ g.fillStyle(PALETTE.path3,0.35); g.fillRect(rint(1,14),rint(1,14),1,1); }
  ...
  makeTex(scene,"tile_water",TILE,TILE,(g)=>{
    ...
    for(let i=0;i<6;i++){ g.fillStyle(PALETTE.flower2,0.18); g.fillRect(rint(2,13),rint(2,13),rint(1,2),1); }
  ...
  makeTex(scene,"tile_fence",TILE,TILE,(g)=>{
    ...
    g.fillRect(2,6,12,2); g.fillRect(2,12,12,2);
    g.fillRect(3,6,2,8); g.fillRect(11,6,2,8);
  ...
  makeTex(scene,"tile_tree",TILE,TILE,(g)=>{
    ...
    g.fillStyle(PALETTE.tree3,0.35); for(let i=0;i<10;i++) g.fillRect(rint(3,12),rint(3,10),1,1);
  ...
  makeTex(scene,"battle_bg_forest",W,H,(g)=>{
    ...
    for (let i=0;i<22;i++){
      const x=rint(0,W), y=rint(40,90), ww=rint(24,46), hh=rint(18,28);
  ...
  makeTex(scene,"bike",48,18,(g)=>{
    ...
    g.fillRect(20,2,6,6);
    g.fillStyle(0xff4d4d,0.45); g.fillRect(30,2,10,8);
...
function makePlayerTex(scene,key,sexo,hairStyle,hairColor,shirtColor,pantsColor,shoeColor,frame){
  ...
  makeTex(scene,key,16,20,(g)=>{
    ...
    g.fillRect(4+legOff,18,3,1); g.fillRect(9-legOff,18,3,1);
    g.fillStyle(pantsColor,1);
    g.fillRect(5+legOff,15,2,3); g.fillRect(9-legOff,15,2,3);
    ...
    const drawHairBase = ()=>{
      ...
      g.fillRect(4,2,8,3);
      if (sexo==="feminino"){ g.fillRect(4,5,2,4); g.fillRect(10,5,2,4); }
    };

    if (hairStyle==="careca"){
      g.fillStyle(PALETTE.ink,0.10); g.fillRect(5,2,6,1);
    } else if (hairStyle==="curto"){
      drawHairBase();
    } else if (hairStyle==="comprido"){
      drawHairBase();
      g.fillStyle(hairColor,1);
      g.fillRect(4,6,2,6); g.fillRect(10,6,2,6); g.fillRect(5,6,6,2);
    } else if (hairStyle==="moicano"){
      g.fillStyle(hairColor,1);
      g.fillRect(7,2,2,7); g.fillRect(6,3,4,2);
    } else if (hairStyle==="black"){
      g.fillStyle(hairColor,1);
      g.fillCircle(8,4,5); g.fillRect(4,4,8,3);
    } else {
      drawHairBase();
    }
...
class TextBox {
  constructor(scene){
    ...
    this.nameText = scene.add.text(SAFE_PAD+10, H-62, "", { fontFamily:"monospace", fontSize:"10px", color:"#d8e0ea" }).setScrollFactor(0);
    this.mainText = scene.add.text(SAFE_PAD+10, H-48, "", { fontFamily:"monospace", fontSize:"10px", color:"#eef3f7", wordWrap:{width:boxW-20} }).setScrollFactor(0);

    this.arrow = scene.add.text(W-SAFE_PAD-18, H-18, "▶", { fontFamily:"monospace", fontSize:"10px", color:"#ffdf7e" }).setScrollFactor(0);
    this.arrow.setVisible(false);

    this.container.add([this.box,this.nameText,this.mainText,this.arrow]);
    this.container.setVisible(false);

    this.active=false; this.lines=[]; this.iLine=0;
    this.full=""; this.shown=""; this.typeSpeed=12; this.timer=0; this.doneLine=false; this.onDone=null;
  }
  start(lines,onDone){
    this.lines=lines; this.iLine=0; this.onDone=onDone||(()=>{});
    this.active=true; this.container.setVisible(true);
    Audio.sfx("ui_ok");
    this.nextLine();
  }
  nextLine(){
    ...
    this.full = line.text || "";
    this.shown=""; this.mainText.setText("");
    this.timer=0; this.doneLine=false; this.arrow.setVisible(false);
  }
  stop(){
    this.active=false; this.container.setVisible(false);
  ...
  advance(){
    ...
    Audio.sfx("ui_move");
    if (!this.doneLine){
      this.shown=this.full; this.mainText.setText(this.shown);
      this.doneLine=true; this.arrow.setVisible(true);
      return;
    }
    this.iLine++;
...
function wipeToScene(scene,nextKey,data){
  ...
  const w=W,h=H;
  const jag=[]; for(let i=0;i<18;i++) jag.push(rint(-6,6));
  scene.tweens.add({
    targets:{t:0}, t:1, duration:360, ease:"Sine.easeInOut",
    onUpdate:(tw,obj)=>{
      const t=obj.t;
      const x=-w + t*(w*2);
      ...
      g.lineTo(x,h); g.closePath(); g.fillPath();
    },
    onComplete:()=> scene.scene.start(nextKey,data)
  });
}
...
function drawHPBar(scene,x,y,w,h,cur,max,depth=2000){
  ...
  g.fillStyle(PALETTE.uiFill2,0.9); g.fillRect(x,y,w,h);
...
class TitleScene extends Phaser.Scene{
  ...
  create(){
    ...
    const logo=this.add.text(W/2,60,"A BATALHA\nDO CLT",{
      fontFamily:"monospace", fontSize:"28px", color:"#eef3f7", align:"center", lineSpacing:-6
    }).setOrigin(0.5);
    logo.setStroke("#000000",6);
    logo.setShadow(2,2,"#000",4,false,true);

    const sub=this.add.text(W/2,106,"Sampa • Diadema",{fontFamily:"monospace",fontSize:"10px",color:"#ffdf7e"}).setOrigin(0.5);
    sub.setShadow(1,1,"#000",2,false,true);

    this.btnBox=this.add.image(W/2,140,"ui_small").setOrigin(0.5).setScale(1.1,1);
    this.btnTxt=this.add.text(W/2,154,"▶ INICIAR",{fontFamily:"monospace",fontSize:"12px",color:"#eef3f7"}).setOrigin(0.5);
    this.btnTxt.setShadow(1,1,"#000",2,false,true);

    this.help=this.add.text(W/2,196,"A/START = confirmar    D-Pad = navegar",{
  ...
  update(){
    // ENTER ou A ou START
    const pad = VPad.get();
    const go = VPad.justDown("a") || VPad.justDown("start") || this.input.keyboard.checkDown(this.input.keyboard.addKey("ENTER"), 0);
    if (go){
      Audio.sfx("ui_ok");
...
/* -------- Character Wizard (igual ao anterior, mas com A/B/D-pad) -------- */
class CreateWizardScene extends Phaser.Scene{
  ...
  create(){
    ...
    this.title=this.add.text(W/2,22,"Criação do Personagem",{fontFamily:"monospace",fontSize:"12px",color:"#ffdf7e"}).setOrigin(0.5);
    ...
    this.swatchLabel=this.add.text(144,136,"",{fontFamily:"monospace",fontSize:"9px",color:"#d8e0ea"}); this.swatchLabel.setShadow(1,1,"#000",2,false,true);
    this.swatchGroup=this.add.container(0,0);
    this.swatchBoxes=[];
    this.swatchName=this.add.text(144,166,"",{fontFamily:"monospace",fontSize:"9px",color:"#eef3f7"}); this.swatchName.setShadow(1,1,"#000",2,false,true);

    this.hint=this.add.text(W/2,202,"D-Pad navegar • A confirma • B volta",{
      fontFamily:"monospace",fontSize:"9px",color:"#d8e0ea"
    }).setOrigin(0.5);
    ...
    t1.setShadow(1,1,"#000",2,false,true); t2.setShadow(1,1,"#000",2,false,true);
    this.modalText=t2;
    this.modal.add([dim,box,t1,t2]);

    this.step=0;
    this.steps=["nome","sexo","cabelo_tipo","cabelo_cor","camiseta_cor","calca_cor","calcado_cor","continuar"];
  ...
  renderSwatches(selectedIndex){
    ...
    const baseX=144, baseY=148, gap=24;
    ...
    for(let i=0;i<visible;i++){
      ...
      rect.setStrokeStyle(2, (idx===selectedIndex)?0xffdf7e:0x92a6bd, (idx===selectedIndex)?0.95:0.55);
  ...
  refresh(){
    ...
    this.sub.setText("A/START para confirmar.");
  }
  ...
  update(){
    ...
    if (this.inModal){
      if (VPad.justDown("left") || VPad.justDown("right")){
        Audio.sfx("ui_move");
        this.modalChoice = 1 - this.modalChoice;
        this.updateModalText();
      }
      if (VPad.justDown("a") || VPad.justDown("start")) this.confirmModal();
      if (VPad.justDown("b")){ Audio.sfx("ui_back"); this.closeModal(); }
      VPad.syncPrev();
      return;
    }

    if (VPad.justDown("up")) this.goStep(-1);
    if (VPad.justDown("down")) this.goStep(1);

    if (VPad.justDown("left")) this.adjust(-1);
    if (VPad.justDown("right")) this.adjust(1);

    if (VPad.justDown("a") || VPad.justDown("start")){
      const k=this.steps[this.step];
      if (k==="nome"){ Audio.sfx("ui_ok"); this.goStep(1); VPad.syncPrev(); return; }
      if (k==="continuar"){ this.openConfirmModal(); VPad.syncPrev(); return; }
      Audio.sfx("ui_ok");
      this.goStep(1);
    }
    ...
    if (VPad.justDown("b")){
      Audio.sfx("ui_back");
      if (this.step===0) wipeToScene(this,"Title",{});
      else this.goStep(-1);
    }
...
/* -------- Overworld / Battle / LevelUp (mesmo comportamento, agora também lê VPad) -------- */
class OverworldScene extends Phaser.Scene{
  ...
  create(){
    ...
    this.player.gridX=6; this.player.gridY=11; this.player.moving=false;
    ...
    if (!state.flags.ato1Done) this.playAct1();
    else this.updateHUD();
  }
  ...
  playAct1(){
    const lines=[
      ...
      {name:"Narrador",text:"(Você pode andar. A/START interage.)"}
    ];
  ...
  isBlocked(x,y){
    ...
    const collides=(spr)=> Phaser.Geom.Rectangle.Contains(spr.getBounds(), px, py);
  ...
  tryMove(dx,dy){
    ...
    this.player.gridX=nx; this.player.gridY=ny;
    ...
    this.tweens.add({
      ...
      onComplete:()=>{
        ...
        if (t===2 && !this.encounterLock && chance(0.16)){
          ...
          this.time.delayedCall(140, ()=> wipeToScene(this,"Battle",{from:"diadema"}) );
        }
      }
    });
  }
  ...
  interact(){
    ...
    if (near(this.npc,24)){
      this.textBox.start([
        ...
        if (!state.flags.metNPC1){ state.flags.metNPC1=true; gainXP(this,18); }
        this.updateHUD();
      });
      return;
    }
    if (near(this.shop1,26)){
      this.textBox.start([{name:"Serralheiro",text:"Arma boa é suor e aço."},{name:"Serralheiro",text:"Volta quando tiver uns trocados."}],()=>{});
      return;
    }
    if (near(this.shop2,26)){
      this.textBox.start([{name:"Drogaria",text:"Cura, mana, alívio temporário."},{name:"Drogaria",text:"Tudo tem seu preço."}],()=>{});
      return;
    }
    if (near(this.shop3,26)){
      this.textBox.start([{name:"Biqueira",text:"Quer um boost?"},{name:"Biqueira",text:"Mas toda escolha cobra depois."}],()=>{});
      return;
    }
    if (near(this.house,26)){
      this.textBox.start([{name:"Narrador",text:"Você volta à casa onde acordou..."},{name:"Narrador",text:"Você descansa um pouco."}], ()=>{
        state.stats.vida=state.stats.vidaMax;
        state.stats.mana=state.stats.manaMax;
        Audio.sfx("heal");
        this.updateHUD();
  ...
  update(time,delta){
    ...
    if (!this.textBox.active){
      if (VPad.justDown("up")) this.tryMove(0,-1);
      else if (VPad.justDown("down")) this.tryMove(0,1);
      else if (VPad.justDown("left")) this.tryMove(-1,0);
      else if (VPad.justDown("right")) this.tryMove(1,0);
    }
...
/* ---- Battle + LevelUp: para manter simples, usamos o mesmo do seu build anterior ---- */
/* (igual ao que você já tinha: HUD safe, moto no fundo do Motoboy, etc.) */
...
class BattleScene extends Phaser.Scene{
  ...
  create(){
    ...
    [this.enemyName,this.enemyHPText,this.playerName,this.playerHPText,this.playerMPText].forEach(t=>t.setShadow(1,1,"#000",2,false,true));
    ...
    this.menuBox.setVisible(false); this.menuText.setVisible(false);
    ...
    this.textBox.start([{name:"Narrador",text:`Um ${this.enemy.name} apareceu!`}], ()=> this.showMenu());
  }
  ...
  hideMenu(){ this.menuBox.setVisible(false); this.menuText.setVisible(false); }
  ...
  renderMenu(){
    ...
    const items=(this.menuMode==="root")?itemsRoot:(this.menuMode==="fight")?itemsFight:itemsMagic;

    const a=items.map((it,i)=>`${(i===this.cursor)?"▶":" "} ${it}`);
    const pad=18;
      ...
      if (this.cursor===2){
        ...
        this.textBox.start([{name:"",text:`Tentando fugir... (${Math.floor(pRun*100)}%)`}], ()=>{
          if (chance(pRun)){
            ...
            this.textBox.start([{name:"",text:"Não deu! O inimigo te cercou!"}], ()=> this.enemyTurn(true));
          }
        });
        return;
      }
  ...
  playerHit(label,res){
    const missOrEvade=()=> this.afterPlayerAttack();

    if (res.fail){
      this.textBox.start([{name:"",text:res.msg}], ()=>{
        this.menuLocked=false; this.showMenu();
      });
      return;
    }
    if (res.miss){
  ...
  update(time,delta){
    ...
    if (this.textBox.active || this.menuLocked){ VPad.syncPrev(); return; }

    if (VPad.justDown("left")) this.moveCursor(-1,0);
    if (VPad.justDown("right")) this.moveCursor(1,0);
    if (VPad.justDown("up")) this.moveCursor(0,-1);
    if (VPad.justDown("down")) this.moveCursor(0,1);

    // SELECT abre STATUS (atalho)
    if (VPad.justDown("select")){
      this.menuMode="root";
      this.cursor=3;
      this.select();
    }
    ...
    if (VPad.justDown("b")){
      if (this.menuMode!=="root"){
        Audio.sfx("ui_back");
        this.menuMode="root"; this.cursor=0; this.renderMenu();
      }
    }
...
class LevelUpScene extends Phaser.Scene{
  ...
  refresh(){
    ...
    if (this.mode==="career"){
      const body=this.optionsCareer.map((o,i)=>`${(i===this.cursor)?"▶":" "} ${o.label}`).join("\n");
      this.list.setText(header + "Escolha seu destino (45+):\n" + body);
      return;
    }
    const body=this.optionsStats.map((o,i)=>`${(i===this.cursor)?"▶":" "} ${o.label}`).join("\n");
  ...
  applyOption(opt){
    ...
    if (this.points<=0){ this.close(); return; }

    Audio.sfx("ui_ok");
    if (opt.key==="vidaMax"){
      s.vidaMax += 5; s.vida = s.vidaMax;
    } else if (opt.key==="inteligencia"){
      s.inteligencia += 1;
      s.manaMax += 2; s.mana = s.manaMax;
    } else {
      s[opt.key] += 1;
    }
    s.vida = s.vidaMax;
    ...
    this.points--;
    this.refresh();
    if (this.points<=0) this.time.delayedCall(160, ()=> this.close());
  }
...
/* =========================================================
   Phaser config (canvas no topo, dentro do #game-shell)
   ========================================================= */
const config = {
  type: Phaser.AUTO,
  width: W,
  ...
  parent: "game-shell", // <<< IMPORTANTÍSSIMO: canvas vai para o topo
  scale: {
    mode: Phaser.Scale.NONE, // escala via CSS no shell (vertical design)
...
// iniciar áudio ao primeiro toque/click (mobile)
window.addEventListener("pointerdown", async ()=>{ if (!Audio.started){ await Audio.start(); Audio.playMusic("serene"); } }, { once:true });
